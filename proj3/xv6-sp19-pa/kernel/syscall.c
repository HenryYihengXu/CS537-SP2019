...
{
  // @1 if(addr >= p->sz || addr+4 > p->sz)
  if(addr >= p->sz || addr+4 > p->sz) // || addr < 4 * PGSIZE) // @1
    return -1;
  *ip = *(int*)(addr);
  return 0;
}
...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
    return -1;
  // @1 if((uint)i >= proc->sz || (uint)i+size > proc->sz)
  if((uint)i >= proc->sz || (uint)i+size > proc->sz)// || (uint)i < 4 * PGSIZE) // @1
    return -1;
  *pp = (char*)i;
  return 0;
}
