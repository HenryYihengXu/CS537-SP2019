...
fork(void)
{
  ...
  // @2 if((np->pgdir = copyuvm(proc->pgdir, proc->sz)) == 0){
    if((np->pgdir = copyuvm(proc->pgdir, proc->sz, proc->top_stack)) == 0){ // @2
    kfree(np->kstack);
    np->kstack = 0;
    np->state = UNUSED;
    return -1;
  ...
  np->sz = proc->sz;
  np->top_stack = proc->top_stack;   // @2
  np->parent = proc;
  *np->tf = *proc->tf;
  np->share[0] = proc->share[0]; // @3
  np->share[1] = proc->share[1]; // @3
  np->share[2] = proc->share[2]; // @3
  
  /*
  if(np->top_stack == USERTOP - PGSIZE)
    cprintf("666\n");
  else
    cprintf("fuck\n");
  */
