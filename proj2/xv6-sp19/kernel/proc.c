...
scheduler(void)
{
  ...
  for(;;){
    ...
    switchuvm(p);
    p->state = RUNNING;
    //cprintf("pname: %s, pid: %d, current: %d, priority: %d, ticks: %d\n", p->name, p->pid, current, p->priority, p->ticks[p->priority]);
...
getstat(struct pstat *stat)
{
  ...
  struct proc *p;
	for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
		...
		int j;
		for (j = 0; j < 4; j++) {
			stat->ticks[i][j] = p->ticks[j];
		}
		for (j = 0; j < 4; j++) {
			stat->wait_ticks[i][j] = p->wait_ticks[j];
		}
		i++;
	}
}
